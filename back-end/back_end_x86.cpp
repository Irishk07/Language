...
Tree_status BackEndX86(Language* language, Labels* labels, const char* name_asm_file, const char* name_bin_file) {
    ...
    CreateAsmFileX86(language, labels, language->tree.root, asm_file, bin_file);
    PrintFuncMyPrintf(language, labels, asm_file, bin_file);
    PrintFuncMyScanf(language, labels, asm_file, bin_file);

    LST_("section .note.GNU-stack noalloc noexec nowrite progbits\n");

    START_(WriteJumpTable(labels, bin_file))
    START_(FinalizeElfSize(bin_file))
...
void CreateAsmFileX86(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file) {
    ...
    if (tree_node->type == OPERATOR) {
        switch(tree_node->value.operators) {
            case OPERATOR_ADD:    PrintOperationsX86(language, labels, tree_node, asm_file, bin_file, "add");      break;
            case OPERATOR_SUB:    PrintOperationsX86(language, labels, tree_node, asm_file, bin_file, "sub");      break;
            case OPERATOR_MUL:    PrintOperationsX86(language, labels, tree_node, asm_file, bin_file, "imul");     break;    
            case OPERATOR_DIV:    PrintOperationDivX86(language, labels, tree_node, asm_file, bin_file);           break;   
            case OPERATOR_SQRT:   PrintOperationSqrtX86(language, labels, tree_node, asm_file, bin_file);          break;  
            case OPERATOR_POW:    PrintOperationPow(language, labels, tree_node, asm_file, bin_file);              break;

            case OPERATOR_ASSIGNMENT: PrintAssignmentX86(language, labels, tree_node, asm_file, bin_file);         break;
            case OPERATOR_CHANGE:     PrintChangeX86(language, labels, tree_node, asm_file, bin_file);             break;

            case OPERATOR_IF:         PrintIfX86(language, labels, tree_node, asm_file, bin_file);                 break;
            case OPERATOR_ELSE:                                                                            break;
            case OPERATOR_WHILE:      PrintWhileX86(language, labels, tree_node, asm_file, bin_file);              break;

            case OPERATOR_COMMON:
            case OPERATOR_PARAM:  CreateAsmFileX86(language, labels, tree_node->left_node, asm_file, bin_file);
                                  CreateAsmFileX86(language, labels, tree_node->right_node, asm_file, bin_file);   break;

            case OPERATOR_INPUT:  PrintInX86(language, labels, tree_node, asm_file, bin_file);                     break;
            case OPERATOR_PRINT:  PrintOutX86(language, labels, tree_node, asm_file, bin_file);                    break;

            case OPERATOR_MAIN_FUNCTION: PrintMainFunctionX86(language, labels, tree_node, asm_file, bin_file);    break;
            case OPERATOR_DEF_FUNCTION:  PrintDefFunctionX86(language, labels, tree_node, asm_file, bin_file);     break;
            case OPERATOR_CALL_FUNCTION: PrintCallFunctionX86(language, labels, tree_node, asm_file, bin_file);    break;
            case OPERATOR_RETURN:        PrintReturnFunctionX86(language, labels, tree_node, asm_file, bin_file);  break;
...
void PrintOperationsX86(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file, const char* operation) {
    ...
    LST_(";/ begin operation %s:\n", operation);

    CreateAsmFileX86(language, labels, tree_node->left_node, asm_file, bin_file);
    CreateAsmFileX86(language, labels, tree_node->right_node, asm_file, bin_file);
...
void PrintOperationPow(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file) {
    ...
    size_t pow_id = language->cnt_labels.cnt_pows++;

    LST_(";/ begin operation pow_%zu:\n", pow_id);

    char label_loop[128] = {};
    snprintf(label_loop, 128, ".loop_pow_%zu", pow_id);
    char label_done[128] = {};
    snprintf(label_done, 128, ".done_pow_%zu", pow_id);
    ...
    MOV_R_NUM_(RAX, 1);
    TEST_R1_R2_(R13, R13);
    uint64_t target_ip = GetLabelAddress(label_done, labels); 
    JZ_(label_done, target_ip);

    LABEL_(label_loop)
    IMUL_R1_R2_(RAX, R12);
    DEC_(R13);
    target_ip = GetLabelAddress(label_loop, labels); 
    JNZ_(label_loop, target_ip);
...
void PrintIfX86(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file) {
    ...
    if (condition->type == OPERATOR && (condition->value.operators == OPERATOR_ABOVE ||
        ...
        CreateAsmFileX86(language, labels, condition->left_node, asm_file, bin_file);
        CreateAsmFileX86(language, labels, condition->right_node, asm_file, bin_file);
        ...
        uint64_t target_ip = GetLabelAddress(jump_label, labels); 
        ...
        CreateAsmFileX86(language, labels, condition, asm_file, bin_file);
        PUSH_NUM_(0);
        ...
        uint64_t target_ip = GetLabelAddress(label_end_if_name, labels);
    ...
    if (tree_node->right_node->type != OPERATOR || tree_node->right_node->value.operators != OPERATOR_ELSE)
        ...
        PrintElseX86(language, labels, tree_node->right_node, asm_file, bin_file, if_id);
...
void PrintElseX86(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file, size_t if_id) {
    ...
    uint64_t target_end = GetLabelAddress(label_end_if_name, labels);
...
void PrintWhileX86(Language* language, Labels* labels, Tree_node* tree_node, FILE* asm_file, FILE* bin_file) {
    ...
    if (condition->type == OPERATOR && (condition->value.operators == OPERATOR_ABOVE ||
        ...
        uint64_t target_end = GetLabelAddress(label_end_while_name, labels);
        if (type_operator == OPERATOR_ABOVE)
    ...
    uint64_t target_begin = GetLabelAddress(label_begin_while_name, labels);
...
void PrintFuncMyPrintf(Language* language, Labels* labels, FILE* asm_file, FILE* bin_file) {
    ...
    MOV_R8_MR_(AL, R12); CMP_R8_CHAR(AL, 0); JE_(DOT_L_(done), GetLabelAddress(DOT_L_(done), labels));
    CMP_R8_CHAR(AL, '%'); JE_(DOT_L_(specificator), GetLabelAddress(DOT_L_(specificator), labels));
    CALL_(L_(print_char)); INC_(R12); JMP_(DOT_L_(check_symbs), GetLabelAddress(DOT_L_(check_symbs), labels));

    DOT_LABEL_(next_symb); INC_(R12); JMP_(DOT_L_(check_symbs), GetLabelAddress(DOT_L_(check_symbs), labels));

    DOT_LABEL_(specificator);
    INC_(R12); MOV_R8_MR_(AL, R12);
    CMP_R8_CHAR(AL, 0); JE_(DOT_L_(done), GetLabelAddress(DOT_L_(done), labels));
    CMP_R8_CHAR(AL, '%'); JE_(L_(case_percent), GetLabelAddress(L_(case_percent), labels));
    CMP_R8_CHAR(AL, 'b'); JL_(L_(case_default), GetLabelAddress(L_(case_default), labels));
    CMP_R8_CHAR(AL, 'x'); JG_(L_(case_default), GetLabelAddress(L_(case_default), labels));
    ...
    FUNC_("mprintf_case_percent"); MOV_R8_CHAR_(AL, '%'); CALL_(L_(print_char)); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_b"); CALL_(L_(get_arg)); MOV_R_NUM_(RCX, 1); MOV_R_NUM_(RSI, 1); MOV_R_NUM_(R9, 64); CALL_(L_(print_deg_2)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_c"); CALL_(L_(get_arg)); CALL_(L_(print_char)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_d"); CALL_(L_(get_arg)); MOVSXD_R1_R2_(RAX, EAX); CALL_(L_(print_decimal)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_f"); MOV_R_ADDR_(R11, ADDR_XMM_SAVE); PUSH_R_(R14); IMUL_R_NUM_(R14, 8); ADD_R1_R2_(R11, R14); MOVQ_R_M_(XMM0, R11); POP_R_(R14);CALL_(L_(print_float)); INC_(R14); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_o"); CALL_(L_(get_arg)); MOV_R1_R2_(RBX, RAX); SHL_(RBX, 1); MOV_R1_R2_(RAX, RBX); MOV_R_NUM_(RCX, 3); MOV_R_NUM_(RSI, 7); MOV_R_NUM_(R9, 21); CALL_(L_(print_deg_2)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_s"); CALL_(L_(get_arg)); MOV_R1_R2_(RDI, RAX); CALL_(L_(print_string)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));
    FUNC_("mprintf_case_x"); CALL_(L_(get_arg)); MOV_R_NUM_(RCX, 4); MOV_R_NUM_(RSI, 15); MOV_R_NUM_(R9, 16); CALL_(L_(print_deg_2)); INC_(R13); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels)); 
    FUNC_("mprintf_case_default"); CALL_(L_(print_char)); JMP_(DOT_L_(next_symb), GetLabelAddress(DOT_L_(next_symb), labels));

    FUNC_("mprintf_get_arg"); CMP_R_NUM_(R13, 5); JL_(DOT_L_(arg_in_regs), GetLabelAddress(DOT_L_(arg_in_regs), labels));
    ...
    MOV_R_ADDR_(R11, ADDR_BUF_LEN); MOV_R_MR_(RDX, R11); CMP_R_NUM_(RDX, 1024); JNE_(DOT_L_(print_to_buf), GetLabelAddress(DOT_L_(print_to_buf), labels)); CALL_(L_(print_buf_to_console));
    ...
    MOV_R_ADDR_(R11, ADDR_BUF_LEN); MOV_R_MR_(RDX, R11); CMP_R_NUM_(RDX, 0); JE_(DOT_L_(pbc_done), GetLabelAddress(DOT_L_(pbc_done), labels)); MOV_R_NUM_(RAX, 1); MOV_R_NUM_(RDI, 1); MOV_R_ADDR_(RSI, ADDR_BUF); SYSCALL_; 
    ...
    FUNC_("mprintf_print_decimal"); PUSH_R_(RBX); TEST_R1_R2_(RAX, RAX); JNS_(DOT_L_(positive_num), GetLabelAddress(DOT_L_(positive_num), labels)); PUSH_R_(RAX); MOV_R8_CHAR_(AL, '-'); CALL_(L_(print_char)); POP_R_(RAX); NEG_(RAX);
    DOT_LABEL_(positive_num); MOV_R_NUM_(RBX, 10); XOR_R1_R2_(RCX, RCX);
    DOT_LABEL_(div_10); XOR_R1_R2_(RDX, RDX); DIV_R_(RBX); ADD_R_NUM_(RDX, '0'); PUSH_R_(RDX); INC_(RCX); CMP_R_NUM_(RAX, 0); JNE_(DOT_L_(div_10), GetLabelAddress(DOT_L_(div_10), labels));
    DOT_LABEL_(print_loop); POP_R_(RAX); PUSH_R_(RCX); CALL_(L_(print_char)); POP_R_(RCX); LOOP_(DOT_L_(print_loop), GetLabelAddress(DOT_L_(print_loop), labels));
    POP_R_(RBX); RET_;

    FUNC_("mprintf_print_float"); RET_;

    FUNC_("mprintf_print_string"); PUSH_R_(RSI); MOV_R1_R2_(RSI, RDI);
    DOT_LABEL_(ps_loop); MOV_R8_MR_(AL, RSI); CMP_R8_CHAR(AL, 0); JE_(DOT_L_(ps_done), GetLabelAddress(DOT_L_(ps_done), labels));
    CALL_(L_(print_char)); INC_(RSI); JMP_(DOT_L_(ps_loop), GetLabelAddress(DOT_L_(ps_loop), labels));
    ...
    JZ_(DOT_L_(zero_digit), GetLabelAddress(DOT_L_(zero_digit), labels)); MOV_R_NUM_(R11, 1); JMP_(DOT_L_(print_digit), GetLabelAddress(DOT_L_(print_digit), labels));
    DOT_LABEL_(zero_digit); CMP_R_NUM_(R11, 1); JE_(DOT_L_(print_digit), GetLabelAddress(DOT_L_(print_digit), labels)); 
    CMP_R_NUM_(R9, 1); JE_(DOT_L_(print_digit), GetLabelAddress(DOT_L_(print_digit), labels)); JMP_(DOT_L_(next_digit), GetLabelAddress(DOT_L_(next_digit), labels));
    DOT_LABEL_(print_digit); MOV_R_ADDR_(R10, ADDR_HEX_CHARS); ADD_R1_R2_(R10, RAX); MOV_R8_MR_(AL, R10); CALL_(L_(print_char));
    DOT_LABEL_(next_digit); DEC_(R9); JNZ_(DOT_L_(deg_loop), GetLabelAddress(DOT_L_(deg_loop), labels));
...
void PrintFuncMyScanf(Language* language, Labels* labels, FILE* asm_file, FILE* bin_file) {
    ...
    DOT_LABEL_(check_sign); MOV_R8_MR_(CL, RSI); CMP_R8_CHAR(CL, '-'); JNE_(DOT_L_(loop), GetLabelAddress(DOT_L_(loop), labels)); MOV_R_NUM_(R13, 1); INC_(RSI);
    DOT_LABEL_(loop); MOVZX_R_M_(RCX, RSI); 
    CMP_R8_CHAR(CL, '0'); JL_(DOT_L_(apply_sign), GetLabelAddress(DOT_L_(apply_sign), labels)); CMP_R8_CHAR(CL, '9'); JG_(DOT_L_(apply_sign), GetLabelAddress(DOT_L_(apply_sign), labels));
    SUB_R_NUM_(RCX, '0'); IMUL_R_NUM_(RAX, 10); ADD_R1_R2_(RAX, RCX); INC_(RSI); JMP_(DOT_L_(loop), GetLabelAddress(DOT_L_(loop), labels));

    DOT_LABEL_(apply_sign); TEST_R1_R2_(R13, R13); JZ_(DOT_L_(store), GetLabelAddress(DOT_L_(store), labels)); NEG_(RAX);
